...
public class UserResource {
    ...
    public ResponseEntity<AdminUserDTO> getUser(@PathVariable @Pattern(regexp = Constants.LOGIN_REGEX) String login) {
        ...
        return ResponseUtil.wrapOrNotFound(userService.getUserWithAuthorityByLogin(login).map(AdminUserDTO::new));
    }
    ...
    public ResponseEntity<Void> deleteUser(@PathVariable @Pattern(regexp = Constants.LOGIN_REGEX) String login) {
        ...
        if (currentLogin.equalsIgnoreCase(login)) {
            // Ném lỗi 400: Không cho xóa
            throw new BadRequestAlertException("Bạn không thể tự xóa tài khoản của chính mình", applicationName, "cannotdeleteself");
        }
        ...
        boolean inPurchaseOrder = purchaseOrderRepository.existsByUserId(userId);
        boolean inSalesOrder = salesOrderRepository.existsByUserId(userId);
        if (inPurchaseOrder || inSalesOrder) {
            throw new BadRequestAlertException("Tài khoản đang nằm trong đơn hàng, không thể xóa", applicationName, "userinorder");
        }
