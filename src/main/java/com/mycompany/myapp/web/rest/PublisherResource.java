...
public class PublisherResource {
    ...
    public ResponseEntity<PublisherDTO> createPublisher(@Valid @RequestBody PublisherDTO publisherDTO) throws URISyntaxException {
        ...
        if (publisherRepository.existsByCodeIgnoreCase(publisherDTO.getCode())) {
            throw new BadRequestAlertException("Nhà xuất bản đã tồn tại", ENTITY_NAME, "codeexists");
        }
    ...
    public ResponseEntity<List<PublisherDTO>> getAllPublishers(@org.springdoc.api.annotations.ParameterObject Pageable pageable) {
        ...
        Page<PublisherDTO> page = publisherService.findAllByStatus(pageable);
    ...
    public ResponseEntity<Void> deletePublisher(@PathVariable Long id) {
        ...
        if (purchaseOrderLineRepository.checkExistsByPublisherId(id) || salesOrderLineRepository.checkExistsByPublisherId(id)) {
            throw new BadRequestAlertException("Không cho xóa vì NXB đang có sách nằm trong đơn hàng", ENTITY_NAME, "publisherinorder");
        }
        boolean hasBooks = bookRepository.existsByPublisherId(id);
        if (hasBooks) {
            throw new BadRequestAlertException("Vẫn còn sách thuộc NXB này, không thể xóa", ENTITY_NAME, "publisherhasbooks");
        }
