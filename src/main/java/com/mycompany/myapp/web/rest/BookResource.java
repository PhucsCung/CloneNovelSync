...
public class BookResource {
    ...
    public ResponseEntity<BookDTO> createBook(@Valid @RequestBody BookDTO bookDTO) throws URISyntaxException {
        ...
        if (bookRepository.existsByCodeIgnoreCase(bookDTO.getCode())) {
            throw new BadRequestAlertException("Tên cuốn sách này đã tồn tại trong hệ thống", ENTITY_NAME, "titleexists");
        }
    ...
    public ResponseEntity<Void> deleteBook(@PathVariable Long id) {
        ...
        Optional<InventoryBalance> balance = inventoryBalanceRepository.findByBookId(id);
        if (balance.isPresent() && balance.get().getQuantityOnHand() > 0) {
            throw new BadRequestAlertException("Sách vẫn còn tồn kho, không thể xóa", ENTITY_NAME, "inventorynotempty");
        }
        boolean inPurchaseOrder = purchaseOrderLineRepository.existsByBookId(id);
        boolean inSalesOrder = salesOrderLineRepository.existsByBookId(id);

        if (inPurchaseOrder || inSalesOrder) {
            throw new BadRequestAlertException("Sách đang nằm trong đơn hàng, không thể xóa", ENTITY_NAME, "bookinorder");
        }
