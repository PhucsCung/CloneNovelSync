...
public class BookRepositoryWithBagRelationshipsImpl implements BookRepositoryWithBagRelationships {
    ...
    Book fetchCategories(Book result) {
        return entityManager
            .createQuery("select book from Book book left join fetch book.categories where book is :book", Book.class)
    ...
    List<Book> fetchCategories(List<Book> books) {
        ...
        List<Book> result = entityManager
            .createQuery("select distinct book from Book book left join fetch book.categories where book in :books", Book.class)
