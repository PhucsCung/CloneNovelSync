<template>
  <div>
    <h2 id="page-heading" data-cy="PurchaseOrderLineHeading">
      <span v-text="$t('cloneNovelSyncApp.purchaseOrderLine.home.title')" id="purchase-order-line-heading">Purchase Order Lines</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('cloneNovelSyncApp.purchaseOrderLine.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'PurchaseOrderLineCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('cloneNovelSyncApp.purchaseOrderLine.home.createLabel')"> Create a new Purchase Order Line </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && purchaseOrderLines && purchaseOrderLines.length === 0">
      <span v-text="$t('cloneNovelSyncApp.purchaseOrderLine.home.notFound')">No purchaseOrderLines found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="purchaseOrderLines">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('quantity')">
              <span v-text="$t('cloneNovelSyncApp.purchaseOrderLine.quantity')">Quantity</span>
            ...
            <th scope="row" v-on:click="changeOrder('unitCost')">
              <span v-text="$t('cloneNovelSyncApp.purchaseOrderLine.unitCost')">Unit Cost</span>
            ...
            <th scope="row" v-on:click="changeOrder('book.title')">
              <span v-text="$t('cloneNovelSyncApp.purchaseOrderLine.book')">Book</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'book.title'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('purchaseOrder.code')">
              <span v-text="$t('cloneNovelSyncApp.purchaseOrderLine.purchaseOrder')">Purchase Order</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'purchaseOrder.code'"></jhi-sort-indicator>
            </th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="purchaseOrderLine in purchaseOrderLines" :key="purchaseOrderLine.id" data-cy="entityTable">
            ...
            <td>{{ purchaseOrderLine.unitCost }}</td>
            <td>
              <div v-if="purchaseOrderLine.book">
                <router-link :to="{ name: 'BookView', params: { bookId: purchaseOrderLine.book.id } }">{{
                  purchaseOrderLine.book.title
                }}</router-link>
              </div>
            </td>
            <td>
              <div v-if="purchaseOrderLine.purchaseOrder">
                <router-link :to="{ name: 'PurchaseOrderView', params: { purchaseOrderId: purchaseOrderLine.purchaseOrder.id } }">{{
                  purchaseOrderLine.purchaseOrder.code
                }}</router-link>
              </div>
            </td>
            <td class="text-right">
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="cloneNovelSyncApp.purchaseOrderLine.delete.question"
      ...
      <div class="modal-body">
        <p id="jhi-delete-purchaseOrderLine-heading" v-text="$t('cloneNovelSyncApp.purchaseOrderLine.delete.question', { id: removeId })">
