<template>
  <div>
    <h2 id="page-heading" data-cy="PurchaseOrderHeading">
      <span v-text="$t('cloneNovelSyncApp.purchaseOrder.home.title')" id="purchase-order-heading">Purchase Orders</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('cloneNovelSyncApp.purchaseOrder.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'PurchaseOrderCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('cloneNovelSyncApp.purchaseOrder.home.createLabel')"> Create a new Purchase Order </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && purchaseOrders && purchaseOrders.length === 0">
      <span v-text="$t('cloneNovelSyncApp.purchaseOrder.home.notFound')">No purchaseOrders found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="purchaseOrders">
        ...
        <tbody>
          <tr v-for="purchaseOrder in purchaseOrders" :key="purchaseOrder.id" data-cy="entityTable">
            ...
            <td v-text="$t('cloneNovelSyncApp.PurchaseStatus.' + purchaseOrder.status)">{{ purchaseOrder.status }}</td>
            <td>{{ purchaseOrder.totalAmount }}</td>
            <td>{{ purchaseOrder.createdAt ? $d(Date.parse(purchaseOrder.createdAt), 'short') : '' }}</td>
            <td>
              {{ purchaseOrder.user ? purchaseOrder.user.login : '' }}
            </td>
            <td class="text-right">
              <div class="btn-group">
                <router-link
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="cloneNovelSyncApp.purchaseOrder.delete.question"
      ...
      <div class="modal-body">
        <p id="jhi-delete-purchaseOrder-heading" v-text="$t('cloneNovelSyncApp.purchaseOrder.delete.question', { id: removeId })">
