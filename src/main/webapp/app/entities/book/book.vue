<template>
  <div>
    <h2 id="page-heading" data-cy="BookHeading">
      <span v-text="$t('cloneNovelSyncApp.book.home.title')" id="book-heading">Books</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('cloneNovelSyncApp.book.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'BookCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-book">
            ...
            <span v-text="$t('cloneNovelSyncApp.book.home.createLabel')"> Create a new Book </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && books && books.length === 0">
      <span v-text="$t('cloneNovelSyncApp.book.home.notFound')">No books found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('code')">
              <span v-text="$t('cloneNovelSyncApp.book.code')">Code</span>
            ...
            <th scope="row" v-on:click="changeOrder('title')">
              <span v-text="$t('cloneNovelSyncApp.book.title')">Title</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'title'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('author')">
              <span v-text="$t('cloneNovelSyncApp.book.author')">Author</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'author'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('retailPrice')">
              <span v-text="$t('cloneNovelSyncApp.book.retailPrice')">Retail Price</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'retailPrice'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('publisher.name')">
              <span v-text="$t('cloneNovelSyncApp.book.publisher')">Publisher</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="cloneNovelSyncApp.book.delete.question" data-cy="bookDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-book-heading" v-text="$t('cloneNovelSyncApp.book.delete.question', { id: removeId })">
