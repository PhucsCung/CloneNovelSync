<template>
  <div>
    <h2 id="page-heading" data-cy="SalesOrderLineHeading">
      <span v-text="$t('cloneNovelSyncApp.salesOrderLine.home.title')" id="sales-order-line-heading">Sales Order Lines</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('cloneNovelSyncApp.salesOrderLine.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'SalesOrderLineCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('cloneNovelSyncApp.salesOrderLine.home.createLabel')"> Create a new Sales Order Line </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && salesOrderLines && salesOrderLines.length === 0">
      <span v-text="$t('cloneNovelSyncApp.salesOrderLine.home.notFound')">No salesOrderLines found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="salesOrderLines">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('quantity')">
              <span v-text="$t('cloneNovelSyncApp.salesOrderLine.quantity')">Quantity</span>
            ...
            <th scope="row" v-on:click="changeOrder('unitPrice')">
              <span v-text="$t('cloneNovelSyncApp.salesOrderLine.unitPrice')">Unit Price</span>
            ...
            <th scope="row" v-on:click="changeOrder('book.title')">
              <span v-text="$t('cloneNovelSyncApp.salesOrderLine.book')">Book</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'book.title'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('salesOrder.code')">
              <span v-text="$t('cloneNovelSyncApp.salesOrderLine.salesOrder')">Sales Order</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'salesOrder.code'"></jhi-sort-indicator>
            </th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="salesOrderLine in salesOrderLines" :key="salesOrderLine.id" data-cy="entityTable">
            ...
            <td>
              <div v-if="salesOrderLine.book">
                <router-link :to="{ name: 'BookView', params: { bookId: salesOrderLine.book.id } }">{{
                  salesOrderLine.book.title
                }}</router-link>
              </div>
            </td>
            <td>
              <div v-if="salesOrderLine.salesOrder">
                <router-link :to="{ name: 'SalesOrderView', params: { salesOrderId: salesOrderLine.salesOrder.id } }">{{
                  salesOrderLine.salesOrder.code
                }}</router-link>
              </div>
            </td>
            <td class="text-right">
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="cloneNovelSyncApp.salesOrderLine.delete.question"
      ...
      <div class="modal-body">
        <p id="jhi-delete-salesOrderLine-heading" v-text="$t('cloneNovelSyncApp.salesOrderLine.delete.question', { id: removeId })">
